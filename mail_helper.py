...
def send_via_emailjs(to_email: str, subject: str, html_content: str, text_content: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            print("Successfully sent email via EmailJS API.")
            return True
        else:
            print(f"EmailJS API returned status code {response.status_code}: {response.text}")
            return False
    except Exception as e:
        print(f"Failed to send email via EmailJS API: {e}")
        return False
...
def send_otp_email(to_email: str, code: str, purpose: str):
    """
    Sends an OTP code via verified HTTP APIs (EmailJS, Resend, Brevo) or SMTP.
    """
    ...
    # 1. Try EmailJS API (Direct Google OAuth SMTP replacement - bypasses all blocks)
    if os.getenv("EMAILJS_SERVICE_ID"):
        if send_via_emailjs(to_email, subject, html_content, text_content):
            return

    # 2. Try Resend API (Only works for account owner without verified domain)
